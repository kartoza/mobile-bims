...
export const allSiteVisits = async () => {
  ...
  return siteVisits.map((siteVisit) => new SiteVisit(siteVisit))
}

export const getSiteVisitsByField = async (field: string, value: any): Promise<SiteVisit[]> => {
  const siteVisits = await load(_SITE_VISITS_KEY)
  const _siteVisits = []
  if (siteVisits) {
...
export const saveSiteVisits = async (siteVisits) => {
