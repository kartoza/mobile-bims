...
import { getWellByField, loadWells, saveWells, updateWellMeasurement, saveWellByField, createNewWell, clearTemporaryNewWells, getWellsByField, removeWellsByField } from "./site.store"
...
it("creates new well", async () => {
  const wells = await loadWells()
  wells.push(new Well({ pk: -1, latitude: 0, longitude: 0, newData: true }))
  wells.push(new Well({ pk: -3, latitude: 0, longitude: 0, newData: true }))
  await saveWells(wells)
  const newWell = await createNewWell(0, 0)
  expect(newWell.pk).toBe(-4)
  expect(newWell.newData).toBe(true)
})
...
it("clears temporary well", async () => {
  ...
  // Check the new wells
  const newWells2 = await getWellsByField('newData', true)
  expect(newWells2.length).toBe(0)
})

it("removes wells by field", async () => {
  const wells = await loadWells()
  wells.push(new Well({ pk: -1, latitude: 0, longitude: 0, newData: false }))
  wells.push(new Well({ pk: -3, latitude: 0, longitude: 0, newData: false }))
  await saveWells(wells)
