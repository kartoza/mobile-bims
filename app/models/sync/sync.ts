...
import Well, { MeasurementType } from "../site/well"
...
// Fetch sites from server, then merge it with local synced data
export const syncPullData = async(setSyncProgress: any, setSyncMessage: any, showError: any) => {
  const sites = await loadSites()
  const api = new Api()
  await api.setup()
  for (let i = 0; i < sites.length; i++) {
    setSyncMessage(`Updating records (${i + 1}/${sites.length})`)
    setSyncProgress((i + 1) / sites.length)
    const apiResult = await api.getWell(sites[i].pk)
    if (apiResult.kind === "ok") {
      sites[i] = apiResult.well
    } else {
