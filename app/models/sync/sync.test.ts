...
import {MeasurementType} from '../site/well';
import {loadWells, saveWells} from '../site/site.store';
import {
  addUnsyncedData,
  getSynced,
  getUnsynced,
...
it('moves unsynced to synced', async () => {
  ...
  expect(allSynced[unsynced.wellPk][0].data.value).toBe(
    unsynced.data.value_value,
  );
