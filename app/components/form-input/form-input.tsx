...
export function FormInput(props: FormInputProps) {
  ...
  const handleChange = (value: string, isUnit = false) => {
  ...
  const pickerForm = (options: any, isUnit = false) => {
    return (
      <View style={[styles.TEXT_INPUT_STYLE, isUnit ? {width: '40%'} : {}]}>
        <Picker
  ...
  return (
    <View>
      <Text
        style={[
          props.required ? styles.LABEL_IMPORTANT : styles.LABEL,
          updated ? {backgroundColor: 'rgba(189, 202, 18, 0.2)'} : {},
        ]}>
        {' '}
        {props.title}
      </Text>
      ...
      <View
        ...
        {props.units ? pickerForm(props.units, true) : <View />}
      </View>
    </View>
  );
}
